...
class ApiService {
  ...
  async postLogin(jsonData: Login): Promise<AxiosResponse<any>> {
    try {
      const response = await axios.post(apiSignInURL, jsonData);
      return response;
    } catch (error: any) {
  ...
  async getProfessions() {
    try {
      const res = await axios.get(`${tempURL}/api/professions`);
      return res.data;
    } catch (error) {
      return error;
    }
  ...
  async getSpecialties() {
    try {
      const res = await axios.get(`${tempURL}/api/specialities`);
  ...
  async getWpImages(kind: string, country?: string) {
    ...
    const response = await axios.get(`${API_URL}/banners${countryParam}`);
